...
│                                                             │
│   ┌───────────┐   ┌────────────────┐   ┌───────────────┐  │
│   │  Customer  │   │Detailer/Owner  │   │  Team Member  │  │
│   │    App     │   │      App       │   │      App      │  │
│   └─────┬─────┘   └───────┬────────┘   └──────┬────────┘  │
│         │                 │                    │            │
│         └─────────────────┼────────────────────┘            │
...
1. **Background jobs** — Move heavy processing from Edge Functions to a dedicated queue (BullMQ or similar)
1. **CDN for media** — Serve job photos and profile images through a CDN rather than direct Supabase Storage reads
1. **Read replicas** — Supabase read replicas for analytics queries that shouldn’t hit the primary database
